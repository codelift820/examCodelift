...
const EditExam = () => {
    ...
    try {
      ...
      if (sectionsData.length === 0) {
        ...
        const formattedSections = sectionsData.map(section => ({
          ...section,
          questions: section.exam_questions.map((q: { id: string; question_text: string; question_order: number; options: string | null; correct_answer?: string; points: number }) => ({
            ...q,
  ...
  navigate(adminRoute("manage-exams"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateQuestion = (sectionId: string, questionId: string, field: keyof ExamQuestion, value: string | number | string[] | undefined) => {
    setSections(prev => prev.map(section => {
      if (section.id === sectionId) {
        return {
          ...section,
  ...
  const updateSection = (sectionId: string, field: keyof ExamSection, value: string | number | ExamQuestion[]) => {
    setSections(prev => prev.map(section => 
      ...
      <header className="border-b border-border bg-card shadow-sm">
        <div className="container mx-auto flex items-center justify-between px-6 py-4">
          <div className="flex items-center gap-4">
            <Button variant="outline" size="sm" onClick={() => navigate(adminRoute("manage-exams"))}>
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => navigate(adminRoute("dashboard"))}> 
